...
| `commandQueue` | Serializes command execution per guild via a promise chain. Prevents race conditions when multiple commands arrive simultaneously.                                                                   |

Most playback and queue commands use both: `@Middlewares(['commandQueue', 'voiceGuard'])`.
...
| Option | Type   | Required | Constraints                                          |
| ------ | ------ | -------- | ---------------------------------------------------- |
| `time` | string | Yes      | Accepts `5.5`, `1m30s`, `1h2m3s`, `HH:mm:ss` formats |

**Middlewares**: commandQueue, voiceGuard

**Flow**: Parses the time string. Validates the position is within track bounds (0 to track length). Seeks. Replies with the formatted target time. Blocks livestreams.
...
| Option | Type   | Required | Constraints                                         |
| ------ | ------ | -------- | --------------------------------------------------- |
| `time` | string | Yes      | Same formats as `/seek`. Added to current position. |
...
| Option     | Type    | Required | Constraints |
| ---------- | ------- | -------- | ----------- |
| `position` | integer | Yes      | Min 1       |

**Middlewares**: commandQueue, voiceGuard

**Flow**: Checks the position is within queue bounds. Removes the track at that position. If out of bounds, replies with an error.
...
Seconds to wait before auto-disconnecting when the queue is empty. Set to 0 to disconnect immediately.
...
### `/config set-queue-add-response-hidden`

| Option  | Type    | Required |
| ------- | ------- | -------- |
| `value` | boolean | Yes      |

Whether `/play` responses are shown as ephemeral (hidden) or visible to the channel.

### `/config set-auto-announce-next-song`

| Option  | Type    | Required |
| ------- | ------- | -------- |
| `value` | boolean | Yes      |

Whether to automatically send a now-playing message when the next track starts.
...
**Flow**: Fetches up to 1000 recent bot messages. Messages less than 14 days old are bulk-deleted (Discord API limit). Older messages are deleted individually. Replies with the count of deleted messages.
...
**Flow**: Checks for duplicate name in the guild. Inserts into the FavoriteQuery table. Unique constraint on (guildId, name).
...
All three buttons perform inline validation (check guild context, player exists, user in same voice channel) rather than using middleware. They call `deferUpdate()` to acknowledge the interaction silently.
