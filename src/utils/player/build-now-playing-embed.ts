...
import { toSafeMarkdownDestination } from '#parlante/utils/general/url';
...
export const buildNowPlayingEmbed = (
...
): { embed: APIEmbed; components: ActionRow<Button>[] } => {
  const current = kPlayer.queue.current;
  if (!current) {
    return {
      embed: {
  ...
  const title = escapeDiscordMarkdown(getTrackTitle(current));
  const author = escapeDiscordMarkdown(current.author ?? 'Unknown Artist');

  // Only a validated http(s) destination becomes a link; anything else renders
  // as plain escaped text. Credentials are stripped and every Markdown-significant
  // character percent-encoded, so a hostile URI can never escape the destination.
  const safeUri = current.uri ? toSafeMarkdownDestination(current.uri) : null;
  const linkedTitle = safeUri ? `[${title}](${safeUri})` : title;

  const playerUI = getPlayerUI(kPlayer);

  const currentRequester = getDiscordUserMention(current.requester);
  let description = `\n**${linkedTitle}**\n\n${messages.embeds.artist}: ${author}`;
  if (currentRequester) description += `\n${messages.embeds.requestedBy}: ${currentRequester}`;
  description += `\n\n${playerUI}`;
  ...
  if (queueSize > 0) {
    ...
    nextSongs.forEach((song, index) => {
      ...
      const requester = getDiscordUserMention(song.requester);
      description += `\n\`${index + 1}.\` ${escapeDiscordMarkdown(truncate(songTitle, getMaxSongTitleLength(songTitle)))} \`[${dur}]\`${requester ? ` · ${requester}` : ''}`;
    });
  ...
  const row = new ActionRow<Button>().addComponents(
    ...
    new Button()
      .setCustomId('player_remove_queue')
      .setEmoji('🗑️')
      .setLabel(messages.queue.removeButton)
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(!hasQueue),
  );
