...
// Discord string option maximum length
...
export default class TtsCommand extends Command {
  async run(ctx: CommandContext<typeof ttsOptions>) {
    ...
    if (message.length > TTS_MAX_MESSAGE_LENGTH) {
    ...
    // Capacity is enforced BEFORE remote synthesis: a full mix queue must not
    // trigger (or let third parties trigger) NodeLink resolve work. Same-guild
    // commands are serialized by commandQueue, so the read stays accurate;
    // addMixLayer below re-checks as defense.
    if (!parlantePlayer.hasTtsCapacity()) {
      await ctx.editOrReply({
