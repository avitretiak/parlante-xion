...
const reportChannelOptions = {
  channel: createChannelOption({
    description: messages.commands.config.setReportChannel.channel,
    required: true as const,
...
@Declare({
  ...
  description: messages.commands.config.setReportChannel.description,
})
@Options(reportChannelOptions)
class SetReportChannelCommand extends SubCommand {
  async run(ctx: CommandContext<typeof reportChannelOptions>) {
    ...
    await ctx.write({
      content: messages.config.updated.reportChannel,
      flags: MessageFlags.Ephemeral,
    });
  }
}
...
class GetConfigCommand extends SubCommand {
  async run(ctx: CommandContext) {
    ...
    const settingsToShow = {
      ...
      [messages.config.labels.reportChannel]: config.reportChannelId
        ? `<#${config.reportChannelId}>`
        : messages.config.labels.notSet,
    };
