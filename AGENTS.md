...
- **Imports**: Use `#parlante/*` subpath imports (defined in `package.json` `"imports"`). Never use relative paths for cross-module imports.
...
- `src/structures/` — Stateful wrappers (ParlantePlayer, Kazagumo init).
...
2-stage build on `oven/bun:1-alpine`. NodeLink runs as separate container (`performanc/nodelink:latest`). YouTube cipher as third container.
